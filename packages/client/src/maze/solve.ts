...
/**
 * BFS shortest path from entrance to exit.
 * Returns array of { row, col } cells in the shortest path, or null if unsolvable.
 */
...
/** Compute the path length in cells */
export function pathLengthCells(path: { row: number; col: number }[]): number {
  return path.length;
}
